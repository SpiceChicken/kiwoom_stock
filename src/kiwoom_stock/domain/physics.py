...
def _calculate_thrust_force(
    execution_strength_pct: float,
    vol_ratio: float,
    interval_amount_krw: float,
    reference_mass: float,
    norm_constant: float = 50.0,
) -> float:
    """
    [물리적 의도: Thrust & Mass]
...
def _calculate_jerk_force(
    current_strength: float,
    prev_strength_5m: float,
    interval_amount_krw: float,
    reference_mass: float,
    norm_constant: float = 20.0,
) -> float:
    """
    [물리적 의도: Jerk (가속도의 미분)]
