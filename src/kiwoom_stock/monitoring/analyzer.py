...
class MarketAnalyzer:
    ...
    def update_priority_supply(self, stock_codes: List[str]):
        ...
        try:
            ...
            for code in stock_codes:
                ...
                self._update_basic_data(code)    # [분리] ka10001 (vol_ratio)
                self._update_strength_data(code) # [분리] ka10046 (strength)
                self._update_alpha_data(code)
                self._update_vwap_data(code)
                self._update_trend_data(code)
    ...
    def _update_basic_data(self, code: str):
        ...
        self.supply_cache[code]['vol_ratio'] = clean_numeric(basic.get('trde_pre', 0.0))

    def _update_strength_data(self, code: str):
        """체결강도 데이터 업데이트"""
        strength = self.collector.fetch_tick_strength(code)
    ...
    def _update_trend_data(self, code: str):
        ...
        try:
            # 1. 60분봉 데이터 수집 (Trend 분석용)
            chart_60m = self.collector.fetch_minute_chart(code, tic="60")
            if not chart_60m:
                return
