...
class MarketDataCollector:
    ...
    def fetch_stock_basic(self, code: str) -> Dict:
        """[ka10001] 주식기본정보요청"""
        try:
            item = self.market_gateway.get_stock_basic_info(code)
            return{
    ...
    def fetch_tick_strength(self, code: str) -> List[Dict]:
        ...
        try:
            items = self.market_gateway.get_tick_strength(code)
            return [
                {
                    key: clean_numeric(value) for key, value in item.items()
                    if not re.search(r'[가-힣a-zA-Z]', str(value))
    ...
    def fetch_minute_chart(self, code: str, tic: str = "1") -> List[Dict]:
        ...
        try:
            items = self.market_gateway.get_minute_chart(code, tic=tic)
            return [
                {
                    key: clean_numeric(value) for key, value in item.items()
                    if not re.search(r'[가-힣a-zA-Z]', str(value))}
    ...
    def fetch_program_trade(self) -> Dict[str, Dict]:
        ...
        try:
            items = self.market_gateway.get_program_trade()
            return {
                item['stk_cd']: {
    ...
    def fetch_foreign_window_trade(self) -> Dict[str, Dict]:
        ...
        try:
            items = self.market_gateway.get_foreign_window_total()
            return {
                item['stk_cd']: {
    ...
    def fetch_order_book(self, code: str) -> Dict[str, Any]:
        try:
            item = self.market_gateway.get_order_book(code)
    ...
    def fetch_recent_ticks(self, code: str) -> List[Dict[str, Any]]:
        try:
            items = self.market_gateway.get_recent_ticks(code)
